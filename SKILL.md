---
name: workday-xslt
description: Workday XSLT and Workday Studio integration expertise. Use when reading, writing, modifying, or debugging XSLT files in a Workday Integrations context - Studio projects, EIB, Core Connectors, PECI/PICOF, Document Transformation, Payroll Interface, or XML-to-CSV/fixed-width/JSON transforms. Trigger on Workday namespaces (wd:, peci:, xtt:, etv:, ps:), debugging empty output, namespace issues, XPath problems, @peci:isDeleted filtering, ETV/XTT attribute transforms, or .xslt/.xsl files in Workday context - even if user only says "Studio", "PECI", "PICOF", or "transformation".
---
